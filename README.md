...
如要使用 **①发表**（自动推送到公众号草稿箱），必须先完成以下微信公众平台配置：

### 1. 获取 API 凭证

1. 登录 [微信公众号后台](https://mp.weixin.qq.com) → **设置与开发** → **基本配置**
2. 记下 **AppID**（开发者 ID）
3. 点击「重置」获取新的 **AppSecret**（开发者密码）→ 保存好，关闭页面后不再显示

### 2. 配置 IP 白名单

在同一个「基本配置」页面，点击 **IP白名单** 的「配置」：

- **直接调用（服务器 IP 固定）** → 添加你的服务器公网 IP
- **通过 CF Worker 代理（IP 不固定）** → 添加 Cloudflare 出口 IP 段：
  - `162.159.98.209`
  - `162.159.99.73`
  - `162.159.100.1`
  - `162.158.0.0/15`
  - `172.64.0.0/13`
- **⚠️ 不支持 IPv6**，只填 IPv4 地址

### 3. 可选：搭建 API 代理

如果服务器 IP 不固定（家庭宽带动态 IP），推荐用 Cloudflare Worker 做转发：

1. 在 CF Dashboard 创建一个 Worker，代码如下（[示例](./examples/worker.js)）
2. 绑定自定义域名（如 `wechat-api.你的域名.com`）
3. 把 Worker 的出口 IP 段加入微信 IP 白名单（见上一步）

> 💡 **纯手动排版（②美化 + ③存档）**不需要微信 API 配置，跳过前置准备即可直接使用排版器。
